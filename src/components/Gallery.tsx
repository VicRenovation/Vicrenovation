...
export const Gallery: React.FC = () => {
  ...
  const getCategoryBadge = (cat: ServiceCategory) => {
    switch (cat) {
      case 'windows':
        return { label: 'Ramen & Deuren', color: 'bg-emerald-100 text-emerald-800 border-emerald-300' };
      case 'interior':
        return { label: 'Interieurrenovatie', color: 'bg-emerald-100 text-emerald-800 border-emerald-300' };
      case 'hsb':
        return { label: 'HSB-panelen', color: 'bg-emerald-100 text-emerald-800 border-emerald-300' };
    }
  };

  return (
    <section id="gallery" className="bg-white text-slate-900 py-16 lg:py-24 border-t border-slate-200">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {filteredItems.length === 0 ? (
          <div className="mt-12 py-16 text-center rounded-2xl border border-slate-200 bg-slate-50 p-8 text-slate-500">
          ...
          <div className="mt-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            return (
              <div
                ...
                className="group relative rounded-2xl overflow-hidden border border-slate-200 bg-white cursor-pointer shadow-sm hover:border-emerald-500/50 hover:shadow-md transition-all duration-300 hover:-translate-y-1"
              >
                {/* Image Container */}
                <div className="h-64 sm:h-72 w-full overflow-hidden relative bg-slate-100 flex items-center justify-center">
                  <img
                    src={item.imageUrl}
                    alt=""
                  ...
                  <img
                    ...
                    loading="lazy"
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-slate-900/60 via-transparent to-transparent opacity-80 group-hover:opacity-60 transition-opacity z-10 pointer-events-none"></div>

                  {/* Badge */}
                  <span className={`absolute top-3 left-3 text-[10px] font-bold px-2.5 py-1 rounded-md border shadow-xs ${badge.color}`}>
                    {badge.label}
                  </span>
                ...
                <div className="absolute bottom-3 left-3 flex items-center gap-1.5 z-20">
                  {extraCount > 0 && (
                    <span className="text-[10px] font-bold px-2.5 py-1 rounded-md bg-slate-900/80 text-white flex items-center gap-1 shadow-md">
                      <Layers className="h-3 w-3 text-emerald-400" />
                      <span>1 + {extraCount} Media</span>
                    </span>
                  )}
                  {item.videoUrl && (
                    <span className="text-[10px] font-bold px-2 py-1 rounded-md bg-emerald-600 text-white flex items-center gap-1 shadow-md">
                  ...
                  {/* Zoom Icon Hover */}
                  <div className="absolute top-3 right-3 h-8 w-8 rounded-lg bg-white/90 border border-slate-200 text-slate-700 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-xs z-20">
      ...
      {activeLightbox && (() => {
        ...
        return (
          <div className="fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-sm flex items-center justify-center p-2 sm:p-4 animate-in fade-in overflow-y-auto">
            <div className="relative max-w-4xl w-full bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-2xl space-y-0 my-auto">
              {/* Close Button */}
              <button
                onClick={() => setActiveLightbox(null)}
                className="absolute top-4 right-4 z-30 h-10 w-10 rounded-full bg-slate-900/80 text-white flex items-center justify-center hover:bg-emerald-600 transition-all shadow-lg"
                title="Sluiten (Esc)"
              >
                <X className="h-5 w-5" />
              </button>
              ...
              <div className="max-h-[55vh] sm:max-h-[65vh] h-[350px] sm:h-[480px] overflow-hidden bg-slate-950 flex items-center justify-center relative group">
                {/* Left / Right Nav Arrows */}
                {mediaList.length > 1 && (
                  <>
                    <button
                      onClick={goPrev}
                      className="absolute left-3 top-1/2 -translate-y-1/2 z-20 h-12 w-12 rounded-full bg-slate-900/80 text-white flex items-center justify-center hover:bg-emerald-600 transition-all shadow-xl border border-slate-700"
                    ...
                    <button
                      onClick={goNext}
                      className="absolute right-3 top-1/2 -translate-y-1/2 z-20 h-12 w-12 rounded-full bg-slate-900/80 text-white flex items-center justify-center hover:bg-emerald-600 transition-all shadow-xl border border-slate-700"
              ...
              {mediaList.length > 1 && (
                <div className="p-4 bg-slate-50 border-t border-slate-200">
                  <div className="text-[11px] font-bold text-slate-500 mb-2.5 uppercase tracking-wider flex items-center gap-1.5">
                      ...
                      return (
                        <button
                          ...
                          className={`relative h-16 w-22 shrink-0 rounded-xl overflow-hidden border-2 transition-all ${
                            isActive
                              ? 'border-emerald-600 ring-2 ring-emerald-500/40 opacity-100 scale-105 shadow-md'
              ...
              <div className="p-6 space-y-3 bg-white border-t border-slate-200">
                ...
                <h3 className="text-xl font-extrabold text-slate-950">{activeLightbox.title}</h3>
                {activeLightbox.description && (
                  <p className="text-xs sm:text-sm text-slate-600 leading-relaxed">{activeLightbox.description}</p>
                )}
              </div>
            </div>
          </div>
