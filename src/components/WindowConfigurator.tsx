...
export const WindowConfigurator: React.FC<WindowConfiguratorProps> = ({ onSelectForQuote }) => {
  ...
  return (
    <section id="configurator" className="bg-slate-50 text-slate-900 py-16 lg:py-24 border-y border-slate-200">
        ...
        <div className="mt-12 grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-5 space-y-4 shadow-sm">
              <label className="text-xs font-bold uppercase tracking-wider text-emerald-700 flex items-center justify-between">
                <span>1. {t('selectProfile')}</span>
                <span className="text-[10px] text-slate-500 font-medium">WDS Profielen</span>
              </label>
                  ...
                  return (
                    <button
                      ...
                      className={`relative text-left p-3.5 rounded-xl border transition-all text-xs ${
                        isSelected
                          ? 'border-emerald-600 bg-emerald-50 text-slate-900 shadow-sm ring-1 ring-emerald-500'
                          : 'border-slate-200 bg-slate-50/70 text-slate-700 hover:border-slate-300 hover:bg-slate-100'
                      }`}
                    >
                      <div className="font-bold text-sm text-emerald-700">{profile.series}</div>
                      <div className="text-[11px] text-slate-500 mt-0.5">{profile.chambers} Kamers • {profile.depthMm}mm</div>
                      <div className="text-[11px] font-semibold text-emerald-700 mt-1">Uw = {profile.uwValue}</div>

                      {isSelected && (
                        <div className="absolute top-2 right-2 h-4 w-4 rounded-full bg-emerald-600 flex items-center justify-center text-white">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-5 space-y-4 shadow-sm">
              <label className="text-xs font-bold uppercase tracking-wider text-emerald-700 flex items-center justify-between">
                ...
                <span className="text-[11px] text-slate-600 font-medium">{selectedColor.name[language]}</span>
              </label>
                ...
                {selectedProfile.availableColors.map((color) => {
                  ...
                  return (
                    ...
                    >
                      {/* Color Swatch Circle */}
                      <div
                        className="h-10 w-10 rounded-lg shadow-inner border border-slate-300/80 relative overflow-hidden transition-transform group-hover:scale-105"
                        style={{
                      ...
                      <span className="text-[10px] text-slate-700 font-medium mt-1.5 line-clamp-1 text-center">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-md relative overflow-hidden">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 pb-4 border-b border-slate-200">
                ...
                <button
                  ...
                  className="flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs font-semibold text-emerald-700 hover:bg-emerald-50 transition-all"
                >
              ...
              <div className="my-6 relative flex items-center justify-center min-h-[380px] rounded-xl bg-slate-100/90 p-8 border border-slate-200 shadow-inner overflow-hidden">
                
                {/* Background Wall Texture Simulation */}
                <div className="absolute inset-0 bg-[radial-gradient(#94a3b8_1px,transparent_1px)] [background-size:16px_16px] opacity-25"></div>

                {/* Simulated Room Interior View through Glass */}
                <div className="absolute inset-12 bg-gradient-to-br from-slate-200/50 via-emerald-100/20 to-slate-200/60 rounded-md pointer-events-none"></div>
                ...
                <div className={`relative transition-transform duration-500 max-w-[340px] sm:max-w-[400px] w-full ${sashOpen ? 'perspective-1000' : ''}`}>
                  <svg
                    ...
                    style={{ filter: 'drop-shadow(0 15px 25px rgba(0,0,0,0.25))' }}
                  >
                    <defs>
                      ...
                      <linearGradient id="glassGrad" x1="0%" y1="0%" x2="100%" y2="100%">
                        ...
                        <stop offset="35%" stopColor="rgba(224, 242, 254, 0.2)" />
                        <stop offset="70%" stopColor="rgba(186, 230, 253, 0.3)" />
                    ...
                    <rect x="10" y="10" width="380" height="300" rx="4" fill="#cbd5e1" stroke="#94a3b8" strokeWidth="3" />
                    ...
                    <rect x="36" y="36" width="328" height="248" fill="none" stroke="rgba(255,255,255,0.25)" strokeWidth="1.5" />

                    {/* Middle Vertical Mullion Profile */}
                    <rect x="193" y="20" width="14" height="280" fill="url(#frameGrad)" stroke="#334155" strokeWidth="1" />

                    {/* Left Fixed Sash Pane */}
                    <g id="leftPane">
                      <rect x="38" y="38" width="153" height="244" fill="url(#frameGrad)" stroke="#334155" strokeWidth="1.5" />
                      {/* Glass */}
                      ...
                      <polygon points="50,48 110,48 50,140" fill="rgba(255,255,255,0.3)" />
                      <polygon points="50,180 180,48 181,80 50,220" fill="rgba(255,255,255,0.15)" />
                    </g>
                    ...
                    >
                      <rect x="209" y="38" width="153" height="244" fill="url(#frameGrad)" stroke="#334155" strokeWidth="2" />
                      {/* Inner Sash Frame Accent */}
                      <rect x="217" y="46" width="137" height="228" fill="none" stroke="rgba(255,255,255,0.3)" strokeWidth="1" />
                      {/* Glass */}
                      <rect x="221" y="50" width="129" height="220" fill="url(#glassGrad)" stroke="#38bdf8" strokeWidth="0.5" strokeOpacity="0.4" />
                      {/* Glass Reflection */}
                      <polygon points="223,50 290,50 223,150" fill="rgba(255,255,255,0.3)" />
                      <polygon points="223,180 348,50 349,80 223,220" fill="rgba(255,255,255,0.15)" />
                    ...
                    {/* Bottom Sill Accent */}
                    <rect x="12" y="296" width="376" height="12" rx="1" fill="#64748b" stroke="#334155" strokeWidth="1" />
                  </svg>
                </div>

                {/* Color Badge Indicator */}
                <div className="absolute bottom-4 left-4 flex items-center gap-2 rounded-lg bg-white/95 border border-slate-200 shadow-sm px-3 py-1.5 text-[11px] font-medium text-slate-800 backdrop-blur-md">
                  <div
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-slate-50 rounded-xl p-4 border border-slate-200 text-center">
                <div>
                  <div className="text-[10px] text-slate-500 uppercase tracking-wider">{t('chambersCount')}</div>
                  <div className="text-base font-black text-emerald-700">{selectedProfile.chambers} Kamers</div>
                </div>

                <div>
                  <div className="text-[10px] text-slate-500 uppercase tracking-wider">{t('depthMm')}</div>
                  <div className="text-base font-black text-slate-900">{selectedProfile.depthMm} mm</div>
                </div>

                <div>
                  <div className="text-[10px] text-slate-500 uppercase tracking-wider">{t('uwValue')}</div>
                  <div className="text-base font-black text-emerald-700">{selectedProfile.uwValue}</div>
                </div>

                <div>
                  <div className="text-[10px] text-slate-500 uppercase tracking-wider">{t('soundDb')}</div>
                  <div className="text-base font-black text-emerald-700">{selectedProfile.soundInsulationDb} dB</div>
                </div>
              </div>
              ...
              <div className="mt-6">
                <button
                  ...
                  className="w-full flex items-center justify-center gap-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 py-4 text-base font-black text-white shadow-lg shadow-emerald-600/25 active:scale-98 transition-all"
                >
